...
const BigRankSingleView = ({ stockCode, winWidth }) => {
    const [isLook, setIsLook] = useState(false);
    const stock = SwrHooks(`${BASE_URL}/ranks/${stockCode}`);
    const router = useRouter();

    const goToDetailPage = useCallback(() => {
        router.push({
            pathname: '/detail',
            query: { shcode: stockCode }
        });
    }, []);
